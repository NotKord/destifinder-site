...
function newMarkerEntryModal() {
    ...
    newPostPhotos = [];
    document.getElementById('newMarkerRating').value = '';
  ...
  try {
    ...
    if (user){
        ...
        const newPost = {
            ...
            ratings: document.getElementById('newMarkerRating').value,
...
async function populateSavedPosts(inputUser) {
    try {
        document.getElementById('savedPostContainer').textContent = 'No Saved Destinations So Far, Get Out There And Find Some!'
        ...
        if (user){
            ...
            if (!Array.isArray(userPosts)) {
                ...
                if(userPosts.length > 0) {
                    ...
                    if (document.getElementById('savedPostContainer').childElementCount === 0) {
                        document.getElementById('savedPostContainer').textContent = 'No Saved Destinations So Far, Get Out There And Find Some!'
                    }
                } else {
                    document.getElementById('savedPostContainer').textContent = 'No Saved Destinations So Far, Get Out There And Find Some!'
